...
class CommonListView extends StatefulWidget {
  const CommonListView({super.key});

  @override
  State<CommonListView> createState() => _CommonListPageState();
}

class _CommonListPageState extends State<CommonListView> {
  ...
  void initState() {
    super.initState();
    context.read<ListBloc>().add(InitStateEvent());
    // _loadItems();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(loc!.commonList),
      ),
      body: BlocBuilder<ListBloc, ListState>(
        builder: (context, state) {
          return Column(
        children: [
          if (state.dataModel.infoMessage != "" && state.dataModel.infoMessage.isNotEmpty)
            Padding(
          padding: const EdgeInsets.all(8.0),
          child: Text(
            state.dataModel.infoMessage,
